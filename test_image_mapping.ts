...
for (const product of testProducts) {
  ...
  console.log(`${status} ${product.padEnd(40)} → ${imageUrl || 'DEFAULT (leche.jpg)'}`);
}

console.log('─'.repeat(100));

// Contar éxitos y fallos
const successful = testProducts.filter(p => getLocalImageUrl(p) !== null).length;
...
console.log(`   📈 Tasa de éxito: ${((successful / testProducts.length) * 100).toFixed(2)}%`);
